...
// ══════════════════════════════════════════════════════════════
// v0.6.3: ILI9341 横屏 320x240 HUD 导航显示
// 屏幕：MSP2807 2.8" SPI ILI9341
// 接线：CS=10 DC=2 RST=4 MOSI=11 SCK=12 BL=6 (HSPI)
//
// v0.6.3: 恢复 sprite 离屏帧缓冲（PSRAM 2MB AP_3v3 已确认正常）
// 320x240x16bit=153KB 分配在 PSRAM，消除直接绘制的画面闪烁。
// ══════════════════════════════════════════════════════════════
...
bool ScreenTFT::init() {
    ...
    mInited = true;
    mSpriteOk = true;
    Serial.printf("[Screen] ILI9341 TFT %dx%d 横屏 (sprite PSRAM)\n", mW, mH);
    return true;
}
